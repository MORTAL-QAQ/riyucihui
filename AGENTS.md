...
- **科研问卷**：6 份表单（卷1 前测 / 卷2 后测·实验组+对照组 / 卷3 实验组附加 / 卷4 认知负荷·实验组+对照组），
  ...
  - **题目源与自检**：题目措辞以 `论文/03_问卷/` 下的《建卷清单》为落笔依据，以《问卷设计（SDT与学习效果）.md》为**权威条目原文**（总表规定「如条目原文有改动，以《问卷设计》为准」）。
    当前 `VERSION=2026-09`：已把 14 行按权威源统一（BPNS 第 1/4/13 行主语、第 10/14 行括号举例、卷3 开放题「你觉得」），
    此前 2026-08 依《建卷清单》落笔；**变更理由与沿革写在 `questionnaires.py` 模块头，改措辞必须升 VERSION 并更新那里**。
    BPNS 三块由 `bpns_blocks(subject)` 生成：卷1 前测主语为「我的日语学习」（前测在平台使用之前），卷2 两版均为网站名——**两版措辞必须一致**。
