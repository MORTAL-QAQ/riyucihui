...
def export_study_report_pdf(
    ...
    """导出学习进度报告为 PDF 文件（#43：渲染逻辑已下沉到 pdf_service）。

    包含：学习统计概览、SM-2 阶段分布、待复习单词列表。
    """
    from ..services.pdf_service import _encode_filename, generate_study_report_pdf

    buf = generate_study_report_pdf(db, user.id)
